...
type Reporte = {
  ...
  fecha: string;             // Fecha de creación (desde backend)
  fechaTerminacion: string | null; // Fecha de terminación (puede ser null)
};

export default function IngMartin() {
  ...
  const fetchReportes = async () => {
    try {
      const response = await fetch(
        'http://192.168.1.160:4000/api/reportes/asesor/Ing Martin'
      );
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.title}>📘 Reportes asignados a Ing. Martin</Text>
