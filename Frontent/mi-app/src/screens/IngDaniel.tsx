...
import { useSocket } from './useSocket'; // Asegúrate de que la ruta sea correcta
...
export default function IngDaniel() {
  ...
  const fetchReportes = async () => {
    try {
      const response = await fetch(
        'http://192.168.1.160:4000/api/reportes/asesor/Ing Daniel'
      );
  ...
  // 🔌 Socket para recibir nuevos reportes en tiempo real
  useSocket((nuevoReporte: Reporte) => {
    if (nuevoReporte.asesor === 'Ing Daniel') {
        ...
        prev.map((r) =>
          r._id === id
            ? {
                ...
                fechaTerminacion: data.reporte.fechaTerminacion,
              }
            : r
        )
      );
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.title}>📘 Reportes asignados a Ing. Daniel</Text>
